...
export default function Home({ checkInRecords, checkOutRecords }) {
  ...
  useEffect(() => {
    ...
    let selectedRecords =
      surveyType === "in" ? checkInRecords : checkOutRecords;
    if (selectedRecords && Object.keys(selectedRecords).length) {
      const recordsOnDay =
        selectedRecords[date.year()][date.month() + 1][date.date()] || [];
      setRecords(
        recordsOnDay.map((record) => {
          return {
            ...record,
    ...
    <div className={classes.root}>
      ...
      <main className={classes.content}>
        ...
        <Box display="flex" alignItems="center" m={1}>
          <Box p={1} flexGrow={1}>
            <Typography variant="h4">{`Survey Data for ${moment(
              selectedDate
        ...
        {isEmpty(checkInRecords) ? null : (
          <SurveyDataTable records={records} surveyType={surveyType} />
        )}
        <ToastContainer />
      </main>
    </div>
