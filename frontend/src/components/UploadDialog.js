...
export default function UploadDialog({ setRefresh }) {
  ...
  return (
    <>
      ...
      >
        <DialogTitle id="upload-dialog">{"Upload a fob data report"}</DialogTitle>
        <DialogContent>
          <FormControl component="fieldset">
            <FormLabel component="legend">Please select the building you are uploading data for:</FormLabel>
            <RadioGroup row aria-label="building" name="building1" value={value} onChange={handleChange}>
              <FormControlLabel value="fnh" control={<Radio />} label="FNH" />
              <FormControlLabel value="mcml" control={<Radio />} label="MCML" />
            </RadioGroup>
          </FormControl>
          {file && <DialogContentText id="upload-dialog-filename">
            Selected file: {file.name}
          </DialogContentText>}
          <Dropzone accept={[".csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]} onDrop={onDrop}>
            {({getRootProps, getInputProps}) => (
              <section>
                <Container {...getRootProps()}>
                  <input {...getInputProps()} />
                  <p>Drag and drop the fob data excel here, or click to select files</p>
                </Container>
              </section>
            )}
          </Dropzone>
