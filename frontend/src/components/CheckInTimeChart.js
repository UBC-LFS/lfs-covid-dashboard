...
export default function CheckInTimeChart({ checkInByTime }) {
        ...
        zoom: {
          ...
          rangeMin: {
            x: dataPoints.length
              ...
              : new Date(moment().startOf("day").format("YYYY-MM-DDTHH:mm")),
          },
          rangeMax: {
            x: dataPoints.length
              ? new Date(dataPoints[dataPoints.length - 1].x)
              : new Date(moment().endOf("day").format("YYYY-MM-DDTHH:mm")),
          },
        },
        pan: {
          enabled: true,
  ...
  return <Scatter height={200} data={data} options={options} />;
}
